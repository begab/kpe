...
/**
 * PorterStemmer.java Programmer: John Keyes (keyeg@ils.unc.edu) Last Modified: January 3, 1998 This class implements
 * the PORTER stemming algorithm, which is fully described in "An algorithm for suffix stripping", M.F. Porter (1980),
...
public class PorterStemmer extends Stemmer {
  ...
  public String stem(String str) {
    ...
    if (str.length() > 3){
      ...
      for (int i = 0; i < c.length; i++){
        if (!Character.isLetter(c[i])){ return str.toLowerCase(); }
      }
    }else{
  ...
  protected String step1b(String str) {
    ...
    if (str.endsWith("eed")){
      ...
      // (*v*) ING ->
    }else if ((str.endsWith("ing")) && (containsVowel(str.substring(0, str.length() - 3)))){ return step1b2(str
        .substring(0, str.length() - 3)); } // end if
    return str;
  } // end step1b
  ...
  protected String step2(String str) {
    ...
    if ((str.endsWith("ational")) && (stringMeasure(str.substring(0, str.length() - 5)) > 0)){
      ...
      return str.substring(0, str.length() - 3) + "e";
      // (m > 0) BILITI -> BLE
    }else if ((str.endsWith("biliti")) && (stringMeasure(str.substring(0, str.length() - 5)) > 0)){ return str
        .substring(0, str.length() - 5) + "le"; } // end if
    return str;
  } // end step2
  ...
  protected String step3(String str) {
    ...
    if ((str.endsWith("icate")) && (stringMeasure(str.substring(0, str.length() - 3)) > 0)){
      ...
      return str.substring(0, str.length() - 3);
      // (m > 0) NESS ->
    }else if ((str.endsWith("ness")) && (stringMeasure(str.substring(0, str.length() - 4)) > 0)){ return str.substring(
        0, str.length() - 4); } // end if
    return str;
  } // end step3
  ...
  protected String step4(String str) {
    if ((str.endsWith("al")) && (stringMeasure(str.substring(0, str.length() - 2)) > 1)){
      ...
      return str.substring(0, str.length() - 3);
      // (m > 1) IZE ->
    }else if ((str.endsWith("ize")) && (stringMeasure(str.substring(0, str.length() - 3)) > 1)){ return str.substring(
        0, str.length() - 3); } // end if
    return str;
  } // end step4
  ...
  protected String step5a(String str) {
    ...
    // (m = 1 and not *0) E ->
        ...
        && (!endsWithCVC(str.substring(0, str.length() - 1))) && (str.endsWith("e")))
      return str.substring(0, str.length() - 1);
    else
      return str;
  } // end step5a
  ...
  protected boolean endsWithDoubleConsonent(String str) {
    ...
    if (c == str.charAt(str.length() - 2))
      if (!containsVowel(str.substring(str.length() - 2))){ return true; }
    return false;
  } // end function
