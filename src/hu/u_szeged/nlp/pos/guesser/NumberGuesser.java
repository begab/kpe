...
import hu.u_szeged.nlp.pos.Util;
...
/**
 * Minden számmal kezdődő token elemzését a NumberGuesser osztály végzi, reguláris kifeje-zések segítségével. Egy
 * szólakhoz több elemzés is tartozhat. Egy számmal kezdődő token lehet főnév (N) (pl.: 386-os@Nc-sn), melléknév
 * (pl.: 16-ai@Afp-sn), számnév (pl. 5.@Mo-snd) vagy nyílt tokenosztályba tartozó (pl.: 20%@Onp-sn).
 */
public class NumberGuesser {
  ...
  private static Pattern PATTERN_1 = Pattern.compile("([0-9]+[0-9\\.,%-/]*-(as|es|os|ös)+)([a-zA-Záéíóöőúüű]*)");

  // 16-i
  private static Pattern PATTERN_2 = Pattern.compile("[0-9]+[0-9\\.,-/]*-i");

  // 16-(ai/ei/jei)
  private static Pattern PATTERN_3 = Pattern.compile("([0-9]+-(ai|ei|jei)+)([a-zA-Záéíóöőúüű]*)");

  // +12345
  private static Pattern PATTERN_4 = Pattern.compile("([\\+|\\-]{1}[0-9]+[0-9\\.,-/]*)-??([a-zA-Záéíóöőúüű]*)");

  // 12345-12345
  private static Pattern PATTERN_5 = Pattern.compile("([0-9]+-[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 12:30 12.30 Ont-sn
  private static Pattern PATTERN_6 = Pattern.compile("(([0-9]{1,2})[\\.:]({1}[0-9]{2}))-??([a-zA-Záéíóöőúüű]*)");

  // 123,45-12345
  private static Pattern PATTERN_7 = Pattern.compile("([0-9]+,[0-9]+-[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 12345-12345,12345
  private static final Pattern PATTERN_8 = Pattern.compile("([0-9]+-[0-9]+,[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 12345,12345-12345,12345
  private static final Pattern PATTERN_9 = Pattern.compile("([0-9]+,[0-9]+-[0-9]+,[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 12345.12345,12345
  private static final Pattern PATTERN_10 = Pattern.compile("([0-9]+\\.[0-9]+,[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 10:30
  private static final Pattern PATTERN_11 = Pattern.compile("([0-9]+:[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 12345.12345.1234-.
  private static final Pattern PATTERN_12 = Pattern.compile("([0-9]+\\.[0-9]+[0-9\\.]*)-??([a-zA-Záéíóöőúüű]*)");

  // 12,3-nak
  private static final Pattern PATTERN_13 = Pattern.compile("([0-9]+,[0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 20-nak
  private static final Pattern PATTERN_14 = Pattern.compile("([0-9]+)-??([a-zA-Záéíóöőúüű]*)");

  // 20.
  private static final Pattern PATTERN_15 = Pattern.compile("(([0-9]+-??[0-9]*)\\.)-??([a-zA-Záéíóöőúüű]*)");

  // 16-áig
  private static final Pattern PATTERN_16 = Pattern.compile("(([0-9]{1,2})-(á|é|jé))([a-zA-Záéíóöőúüű]*)");

  // 16-a
  private static final Pattern PATTERN_17 = Pattern.compile("(([0-9]{1,2})-(a|e|je))()");

  // 50%
  private static final Pattern PATTERN_18 = Pattern.compile("([0-9]+,??[0-9]*%)-??([a-zA-Záéíóöőúüű]*)");
  ...
  private static String nounToNumeral(String nounMsd, String numeralMsd) {
    ...
    return MagyarlancResourceHolder.getKRToMSD().cleanMsd(msd.toString());
  }
    ...
    if (matcher.matches()) {
      ...
      if (suffix.length() > 0)
        for (MorAna stem : MorPhonGuesser.guess(root, suffix)) {
          ...
          stemSet.add(new MorAna(root, stem.getMsd().replace("Nc", "Afp")));
        }
    ...
    if (matcher.matches()) {
      ...
      if (suffix.length() > 0) {
        for (MorAna stem : MorPhonGuesser.guess(root, suffix)) {
          ...
          if (Util.isDate(matcher.group(2))) {
            stemSet.add(new MorAna(root + ".", nounToNoun(stem.getMsd(), "Nc------s3-")));
          }

          if (matcher.group(3).equals("é")) {
      ...
      if (stemSet.size() == 0) {
        ...
        if (Util.isDate(matcher.group(2))) {
          stemSet.add(new MorAna(matcher.group(2) + ".", "Nc-sn---s3"));
        }
      }
  ...
  public static void main(String[] args) {
    System.out.println(NumberGuesser.guess("386-os"));
